...
enum RESERVED
{
  ...
  TK_BEGIN,
  TK_EXTENDS,
  TK_INSTANCEOF, // New narrow keywords.
  ...
  TK_EXPORT,
  TK_TRY,
  TK_CATCH, // New non-compatible keywords.
  ...
  TK_SUGGEST_0,
  TK_SUGGEST_1, // New special keywords.
#endif
  TK_RETURN,
  TK_THEN,
  TK_TRUE,
...
struct Token
{
  ...
  {
    return IsReserved() && token != TK_TRUE && token != TK_FALSE && token != TK_NIL && token != TK_PPARENT && token != TK_PARENT;
  }

  [[nodiscard]] bool IsNarrow() const noexcept
  {
    return token == TK_IN || (token >= TK_CASE && token < FIRST_COMPAT);
  }
...
struct LexState
{
  ...
  {
    if (!tokens.empty() && tokens.back().token != TK_EOS)
    { /* doing lexer pass? */
  ...
  [[nodiscard]] bool shouldEmitWarning(int line, WarningType warning_type) const
  {
    ...
    return lastattr.find("@pluto_warnings: disable-next") == std::string::npos && lastattr.find("@pluto_warnings disable-next") == std::string::npos && getWarningConfig().isEnabled(warning_type);
  }
