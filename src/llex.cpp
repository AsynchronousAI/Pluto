...
static int read_numeral(LexState *ls, SemInfo *seminfo)
{
  ...
  for (;;)
  {
    ...
    else if (lisxdigit(ls->current) || ls->current == '.' || ls->current == '_')
    { /* '%x|%.' */
      if (ls->current != '_')
        save_and_next(ls);
      else
...
static void process_string_escape(LexState *ls)
{
  ...
  case '{':
    ...
    goto read_save;
  case EOZ:
    return; /* no save, will raise an error next loop */
  case 'z':
...
static int llex(LexState *ls, SemInfo *seminfo, int *column)
{
  ...
  for (;;)
  {
    ...
    case '/':
    {
      ...
      else
      {
        if (check_next1(ls, '/'))
        {
          ...
          { /* floor division compound support */
            ls->appendLineBuff("//=");
            seminfo->i = TK_IDIV;
            return '=';
          }
    ...
    case '_':
    { /* arbitrary underscores for more readable long numbers */
      ...
      { /* needed to emulate default check because _ is fairly multi-purpose. */
        next(ls);
    ...
    case '*':
    { /* special case compound, need to support mul, exponent, and augmented mul */
      next(ls);
      if (check_next1(ls, '='))
      {
        ls->appendLineBuff("*=");
