...
static l_noret throwerr(LexState *ls, const char *err, const char *here, int line)
{
  ...
  msg->addMsg(err);
  if (ls->t.token == TK_EOS && strstr(err, "near '<eof>'") == nullptr)
  { /* for 'incomplete' in REPL */
    msg->addMsg(" near ")
...
{
  return ls->t.token == TK_NAME || ls->t.IsNarrow() || ((flags & N_RESERVED_NON_VALUE) && ls->t.IsReservedNonValue()) || ((flags & N_RESERVED) && ls->t.IsReserved()) || ((flags & N_OVERRIDABLE) && ls->t.IsOverridable());
}
...
static void exp_propagate(LexState *ls, const expdesc &e, TypeHint &t) noexcept
{
  ...
  else if (e.k == VCONST)
  {
    ...
    case LUA_TNIL:
      ...
      break;
    case LUA_TBOOLEAN:
      t.emplaceTypeDesc(VT_BOOL);
      break;
    case LUA_TNUMBER:
      t.emplaceTypeDesc((ttypetag(val) == LUA_VNUMINT) ? VT_INT : VT_FLT);
      break;
    case LUA_TSTRING:
      t.emplaceTypeDesc(VT_STR);
      break;
    case LUA_TTABLE:
      t.emplaceTypeDesc(VT_TABLE);
      break;
    case LUA_TFUNCTION:
      t.emplaceTypeDesc(VT_FUNC);
      break;
    }
  }
}
...
static void adjust_assign(LexState *ls, int nvars, int nexps, expdesc *e)
{
  ...
  if (hasmultret(e->k) || e->k == VSAFECALL)
...
static void statlist(LexState *ls, TypeHint *prop = nullptr, bool no_ret_implies_void = false)
{
  ...
  if (prop)
  {             /* do we need to propagate the return type? */
    if (!ret && /* had no return statement? */
        no_ret_implies_void)
    {                                 /* does that imply a void return? */
...
static void field(LexState *ls, ConsControl *cc, bool for_class = false)
{
  ...
  if (ls->t.IsReserved() && luaX_lookahead(ls) == '=')
  {
    ...
    {
    case TK_NAME:
    { /* may be 'listfield', 'recfield' or static 'funcfield' */
...
static void checkrettype(LexState *ls, TypeHint &rethint, TypeHint &retprop, int line)
{
  if (!rethint.empty()                                         /* has type hint for return type? */
      ...
      && !rethint.isCompatibleWith(retprop))
  { /* incompatible? */
...
static void funcargs(LexState *ls, expdesc *f, TypeDesc *funcdesc = nullptr)
{
  ...
  if (funcdesc)
  {
    ...
    if (!funcdesc->proto->is_vararg && expected < received)
    {                                         /* Too many arguments? */
...
static void const_expr(LexState *ls, expdesc *v)
{
  ...
  case TK_NAME:
  {
    ...
    else if (!check_constexpr_call(ls, v, "tonumber", luaB_tonumber) && !check_constexpr_call(ls, v, "utonumber", luaB_utonumber) && !check_constexpr_call(ls, v, "tostring", luaB_tostring) && !check_constexpr_call(ls, v, "utostring", luaB_utostring))
    {
  ...
  for (;;)
  {
    ...
    { /* funcargs */
      ...
      else if (v->k == VUPVAL)
      {
        ...
        int idx = uv->idx;
        while (efs)
        { /* efs == nullptr && instack if resolving _ENV */
          if (uv->instack)
          {
...
static void switchimpl(LexState *ls, int tk, void (*caselist)(LexState *, void *), void *ud = nullptr)
{
  ...
  if (ls->laststat.token != TK_BREAK)
  { /* last block did not have 'break'? */
    if (tk == ':')
    { /* switch statement? */
...
static void switchexpr(LexState *ls, expdesc *v)
{
  ...
  switchimpl(ls, TK_ARROW, [](LexState *ls, void *ud)
    ...
    expr(ls, &cv);
    luaK_exp2reg(ls->fs, &cv, reg);
    lbreak(ls, 1, line, luaK_jump(ls->fs)); }, v);
}
...
static BinOpr getbinopr(int op)
{
  ...
  case TK_COAL:
    return OPR_COAL;
  case TK_POW:
    return OPR_POW; /* '**' operator support */
  default:
    return OPR_NOBINOPR;
  }
}
...
static void prefixplusplus(LexState *ls, expdesc *v, bool as_statement)
{
  ...
  expdesc e = *v, v2;
  if (v->k != VLOCAL)
  { /* complex lvalue, use a temporary register. linear perf incr. with complexity of lvalue */
  ...
  { /* simple lvalue; a local. directly change value (~20% speedup vs temporary register) */
    enterlevel(ls);
    luaK_infix(fs, OPR_ADD, &e);
    init_exp(&v2, VKINT, 0);
    v2.u.ival = 1;
...
static BinOpr subexpr(LexState *ls, expdesc *v, int limit, TypeHint *prop, int flags)
{
  ...
  { /* disambiguate '+' operator and '++' as next statement */
    leavelevel(ls);
    return OPR_NOBINOPR;
  }
...
static void expr(LexState *ls, expdesc *v, TypeHint *prop, int flags)
{
  ...
  { /* ternary expression? */
    ...
    if (prop)
      prop->clear(); /* we don't care what type the condition is/was */
    int escape = NO_JUMP;
    v->normalizeFalse();
...
static void enumstat(LexState *ls)
{
  ...
  if (gett(ls) != TK_BEGIN)
  { /* enum has name (and possibly modifier)? */
    if (ls->t.token == TK_CLASS || (ls->t.token == TK_NAME && strcmp(getstr(ls->t.seminfo.ts), "class") == 0))
    {
      is_enum_class = true;
      luaX_next(ls);
    }
  ...
  while (gett(ls) != TK_END)
  {
    ...
    if (testnext(ls, '='))
    {
      ...
      { /* compile-time constant? */
        ...
        if (ttype(k) == LUA_TNUMBER && ttisinteger(k))
        { /* integer value? */
          init_exp(&v, VKINT, 0);
          v.u.ival = ivalue(k);
        }
...
static void test_then_block(LexState *ls, int *escapelist, TypeHint *prop, bool &has_and)
{
  ...
  if (ls->t.token == TK_BREAK && luaX_lookahead(ls) != TK_INT)
  { /* 'if x then break' and not 'if x then break int' ? */
...
static void localstat(LexState *ls)
{
  ...
  do
  {
    ...
    else if (kind == VDKREG)
    {
      ...
      {
        if (strcmp(getstr(ls->t.seminfo.ts), "const") == 0 || strcmp(getstr(ls->t.seminfo.ts), "constexpr") == 0 || strcmp(getstr(ls->t.seminfo.ts), "close") == 0)
        {
  ...
  if (testnext(ls, '='))
  {
    ...
    {
      if (ls->localstat_expression_names.find(variable_name) == ls->localstat_expression_names.end())
      {                                                              // Not a localization optimization?
...
static void exprstat(LexState *ls)
{
  ...
  struct LHS_assign v;
  suffixedexp(ls, &v.v);
  if (ls->t.token == '=' || ls->t.token == ',' || ls->t.token == TK_NE)
  { /* stat -> assignment ? */
    v.prev = NULL;
...
static void retstat(LexState *ls, TypeHint *prop)
{
  ...
  int startpc = fs->pc;
  int endpc = -1;
  if (block_follow(ls, 1) || ls->t.token == ';' || (!ls->switchstates.empty() && (ls->t.token == TK_CASE || ls->t.token == TK_DEFAULT)))
  {
    nret = 0; /* return no values */
    if (prop)
      prop->emplaceTypeDesc(VT_VOID);
  }
  else
  {
    if (fs->istrybody)
    ...
    if (hasmultret(e.k))
    {
      ...
      if (e.k == VCALL && nret == 1 && !fs->bl->insidetbc && !fs->istrybody)
      { /* tail call? */
  ...
  do
  {
    ...
    else if (ls->t.token == TK_STRING)
    {
      ...
      else if (soup::version_compare(getstr(ls->t.seminfo.ts), "0.2.0") >= 0)
      {
        ...
        throwerr(ls, luaO_fmt(ls->L, "'pluto_use \"%s\"' is not valid", getstr(ls->t.seminfo.ts)), "did you mean \"0.8.0\", \"0.6.0\", \"0.5.0\" or \"0.2.0\"?");
...
static void trystat(LexState *ls)
{
  ...
  Labellist *gl = &ls->dyd->gt;
  if (bl.firstgoto != gl->n)
  {                                                /* There are some gotos that need to be moved out of the try/catch block */
    ...
    for (int i = bl.firstgoto; i < gl->n; i++)
    { /* for each pending goto */
      Labeldesc *gt = &gl->arr[i];
      ...
      { /* Needs patching up */
        ...
        { /* Look for other gotos with the same name and patch them together */
  ...
  int num_out = new_fs.seenrets & (1 << 2) ? 4 : new_fs.seenrets & (1 << 1) ? 3
                                                                            : 2;
...
static void importstat(LexState *ls)
{
  /* stat -> IMPORT {NAME [AS NAME]},... [FROM NAME] */
  Import imports[MAX_IMPORTS];
  TString *from = nullptr;

  int nimports = 0;
  while (ls->t.token == TK_NAME)
  {
    /* handle import counter */
    if (nimports == MAX_IMPORTS)
      throwerr(ls, "too many imports", "maximum number of imports exceeded", ls->getLineNumber());
    nimports++;

    /* add import */
    imports[nimports].name = str_checkname(ls);
    imports[nimports].alias = imports[nimports].name;

    /* check alias */
    if (testnext(ls, TK_AS))
    {
      imports[nimports].alias = str_checkname(ls);
    }
  ...
  if (testnext(ls, TK_FROM))
    from = str_checkname(ls);

  /******* Log */
  printf("Importing %d modules:\n", nimports);
  printf("  FROM: %s\n", from ? getstr(from) : "nil");
  for (int i = 1; i <= nimports; i++)
  {
    printf("  %s", getstr(imports[i].name));
    printf(" AS %s", getstr(imports[i].alias));
    printf("\n");
  }
}
...
static void statement(LexState *ls, TypeHint *prop)
{
  ...
  if (ls->t.token != ';')
  {
    if (ls->laststat.IsEscapingToken() || (ls->laststat.Is(TK_GOTO) && ls->t.token != TK_DBCOLON)) /* Don't warn if this statement is a goto label. */
    {
      throw_warn(ls,
                 "unreachable code",
